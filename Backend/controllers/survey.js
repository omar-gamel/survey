...
export default {
    ...
    async getCount(req, res, next) {
      ...
      try {
        ...
        survey.forEach(s => {
          s.answer.find(q => { 
             if( q.question == questionId &&  q.choose == 5)
             veryAgree++;
          });
          s.answer.find(q => { 
            if( q.question == questionId &&  q.choose == 4)
            agree++;
          });
          s.answer.find(q => { 
            if( q.question == questionId &&  q.choose == 3)
            neutral++;
          });
          s.answer.find(q => { 
            if( q.question == questionId &&  q.choose == 2)
            disAgree++;
          });
          s.answer.find(q => { 
            if( q.question == questionId &&  q.choose == 1)
            veryDisagress++;
          });
        })
        res.send({ result: [veryAgree, agree, neutral, disAgree, veryDisagress], count: count });
      } catch (error) {
        next(error);
      }
    }
