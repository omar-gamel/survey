...
import { validateUser, validateChangePassword } from '../services/validation/user';
...
export default {
  ...
  async login(req, res, next) {
    try {
      const user = await User.findOne({ email: req.body.email });
      if (!user) 
          return res.send({success: false, message: 'Email or password is not valid'});  
  ...
  async getUser(req, res, next) {
    try {
      ...
      if (!user) 
          throw new ApiError(404, { message: `user with id : ${req.user._id} is not found` });
  ...
  async getAdminUser(req, res, next) {
    try {
      ...
      if (!user) 
        throw new ApiError(404, { message: `user with id : ${req.params.userId} is not found` });
  ...
  async update(req, res, next) {
    ...
    try {
      ...
      if(!user)
          return res.send({ success: false, message: 'No user found with the provided id' });
  ...
  async changePassword(req, res, next) {
    ...
    try {
      ...
      await User.findByIdAndUpdate(req.user._id, { password: req.body.newPassword }, { new: true })   
  ...
  async AnsweredSuject(req, res, next) {
    try {
      await User.findByIdAndUpdate(req.user._id, { $addToSet: { isAnswered: req.body.subjectId }} , { new: true });
