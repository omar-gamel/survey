...
export default {
    ...
    async getById(req, res, next) {
      try {
        ...
        if (!question) 
            return res.send({  success: false,  message: `question with id :${req.params.questionId} is not found` });
        res.send(question);
      } catch (error) {
        next(error);
      }
    ...
    async update(req, res, next) {
      ...
      try {
        ...
        if(!question)
            return res.send({ success: false, message: 'No question found with the provided id' });
        ...
        await Question.findByIdAndUpdate(questionId, updateQuestion, { new: true });
    ...
    async delete(req, res, next) {
      ...
      try {
          const question = await Question.findById(questionId);
          if (!question) 
              return res.status(400).send({ success: false, message: 'Question not Found.' });
